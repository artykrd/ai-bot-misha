...
class VeoService(BaseVideoProvider):
    ...
    async def _generate_veo_video(
        ...
        try:
            ...
            if input_video_path:
                try:
                    uploaded_video_file = await asyncio.to_thread(
                        self.client.files.upload, file=input_video_path
                    )
            ...
            operation = await gemini_execution_layer.execute(
            ...
            while not operation.done:
                ...
                # Refresh operation status (sync call, run in thread)
            ...
            # Download and save video file (sync calls, run in thread)
