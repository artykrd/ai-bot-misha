...
### #A5 Veo 3.1 отключён (исчерпана квота Google Gemini)
**Симптом (от пользователя):** «veo перестал работать, люди жалуются».
**Диагностика по логам:** после фикса #A3 (бамп google-genai) запрос стал корректно уходить в
Google (`veo_image_prepared`), но Google отвечает **`429 RESOURCE_EXHAUSTED`** —
«You exceeded your current quota» (17:21–17:23, после 5 ретраев). То есть код исправен, но у
Gemini-ключа закончилась квота/биллинг на Veo.
**Рассмотренный вариант:** перевод Veo на kie.ai (`POST /api/v1/veo/generate`, poll
`GET /api/v1/veo/record-info`, модели `veo3`/`veo3_fast`/`veo3_lite`) — kie.ai уже используется
в боте (Midjourney, Nano Banana) и `KIE_API_KEY` настроен. **Решение пользователя:** не мигрировать,
а **отключить Veo и убрать кнопку**; Motion Control оставить на официальном Kling API (не трогать).
**Фикс:**
- `keyboards/inline.py`: убран `("Veo 3.1","bot.veo")` из `MENU_BUTTONS`; убрана кнопка Veo из
  `create_video_keyboard` (Hailuo переставлен к Kling, Luma отдельной строкой).
- `media_handler.py`: убрана кнопка `photo_video:veo` (осталось Kling+Luma) и строка про Veo в
  подписи; добавлена константа `VEO_UNAVAILABLE_TEXT`; хэндлер `bot.veo` и guard в
  `photo_video:veo` теперь показывают «временно недоступно» (для закэшированных кнопок).
- `common.py`: команда `/veo` показывает то же сообщение «временно недоступно».
- Google-путь `VeoService` и `MODEL_ACTIONS["veo"]` оставлены как есть (не вызываются), чтобы
  быстро вернуть модель при восстановлении квоты/смене провайдера.
**Проверка:** AST/импорт OK; `veo` отсутствует в `MENU_BUTTONS`; после рестарта — 0 ошибок,
polling + video_worker(8s), `/health`=200.
**Статус:** ✅ Сделано (Veo отключён до восстановления лимитов / переезда на kie.ai)

### #A6 Kling Motion Control: ошибка 1201 при загрузке видеофайлом
**Симптом:** при прикреплении видео файлом Motion Control падал с
`Kling API error: 400 - {"code":1201,"message":"Something went wrong when we tried to get the
contents of the file."}` (подтверждено в логах сегодня 14:19 и 17:20).
**Причина:** `kling_mc_receive_video_file` формировал `video_url` как прямую Telegram-ссылку
(`api.telegram.org/file/bot<token>/<path>`), которую серверы Kling не могут скачать. (Картинка
уходит как base64 — с ней проблем нет; ломалось только видео.)
**Фикс (Motion Control остаётся на официальном Kling API):**
- Новый модуль `app/services/file/kie_upload.py` — `upload_bytes_to_kie()`: заливает байты на
  публичный хостинг `kieai.redpandaai.co/api/file-stream-upload` и возвращает публичный URL
  (тот же хост, который Kling уже успешно скачивает для Nano Banana картинок).
- `kling_mc_receive_video_file`: скачиваем видео из Telegram → заливаем на Kie → в стейт кладём
  публичный URL. Telegram-URL больше не используется. При ошибке заливки — честная ошибка
  пользователю (без отката на нерабочую ссылку), с подсказкой прислать прямой URL.
**Проверка:** AST/импорт OK; реальная тестовая заливка вернула рабочий URL
(`tempfile.redpandaai.co/...mp4`), `curl` → HTTP 200 (публично доступен); после рестарта —
0 ошибок, polling + video_worker(8s), `/health`=200.
**Не проверено сквозно:** фактическая склейка видео Kling'ом — это можно подтвердить только на
реальном видео пользователя + списании токенов. Механика URL (заливка + публичная доступность)
проверена.
**Статус:** ✅ Сделано (закрывает #13.8)
...
- [x] 13.8 **Kling Motion Control 1201**: ✅ ИСПРАВЛЕНО (#A6). Видео загруженного файла теперь
  заливается на публичный Kie.ai-хостинг и в Kling уходит этот URL вместо Telegram-ссылки.
